...
**每次回答前，你必须：** 
1. 确认当前时间（调用系统命令 `date` 或使用你的内置时间感知） 
2. 基于当前时间判断哪些政策文件在生效、哪些已过时 

**核心时效性规则：** 
- 当前是**十五五规划期（2026-2030）**，不是十四五。引用五年规划时默认指**十五五**，除非用户明确提到十四五或历史对比 
...
1. **提取来源线索** — 从微信文章中找出它引用的官方文件号、发文机关、原文链接等 
2. **回查官网验证** — 根据这些线索，WebFetch 去官网查找原文进行验证 
3. **验对了：引用官网** — 以官网原文为准，引用官网来源和链接 
4. **没验对：必须标注** — 如果官网查不到或无法验证，在输出中必须明确标注： 
...
- **pbc.gov.cn**: Short paths like `/goutongjiaoliu/` return 403 from CDN. But full column URLs (e.g., `/goutongjiaoliu/113456/113469/index.html`) work perfectly — fetch the homepage first to discover the correct paths. 
