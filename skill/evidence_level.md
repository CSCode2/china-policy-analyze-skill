...
The evidence level system (E0-E5) classifies the strength of evidence supporting a claim. Every substantive claim in an answer must be tagged with its evidence level. This allows the reader to immediately assess how well-supported each claim is.
...
**Restrictions**:
- E0 claims must never be presented as conclusions
- E0 claims must never influence action suggestions
- If a key claim is E0, the answer must state that the evidence is insufficient

**Example**:
> Claim: "The government may introduce new subsidies for the semiconductor industry in Q3."
> Evidence Level: E0 — No official document or credible source supports this specific timeline. This is speculative.
...
**Restrictions**:
- E1 claims cannot be used to support action suggestions without strong qualification
- E1 claims must note the indirect nature of the evidence
- Multiple E1 claims cannot be combined to create the appearance of E2+ evidence

**Example**:
> Claim: "Several provinces are likely to implement the central carbon policy based on their previous track records."
> Evidence Level: E1 — Based on E-level analysis and historical patterns, not on actual provincial implementation documents.
...
**Restrictions**:
- E2 claims about central policy direction are acceptable but should note the single-source limitation
- E2 claims about local implementation should be treated cautiously — one document does not confirm actual implementation
- E2 claims that could be affected by newer documents should note the recency check status

**Example**:
> Claim: "The 14th Five-Year Plan identifies quantum computing as a strategic frontier."
> Evidence Level: E2 — Directly supported by the 14th FYP text. No contradiction from other sources, but the claim is based on a single document.

---

### E3: Multi-Source Convergent Evidence
...
- E3 is generally sufficient for most policy claims
...
> Evidence Level: E3 — Supported by the 14th FYP (S-level), NDRC implementation opinions (A-level), and MOF subsidy notices (A-level).

---

### E4: Multi-Source Convergent Evidence with Implementation Verification

**Definition**: The claim is supported by E3-level evidence PLUS evidence of actual implementation (funding disbursed, projects approved, enforcement actions taken, pilots operational).

**When to use**:
- E3 evidence exists AND there is C-level evidence of actual implementation
- E3 evidence exists AND there are enforcement cases confirming the policy is being executed
- E3 evidence exists AND official data confirms the policy effect
...
- E4 is the minimum level for confident claims about what is actually happening on the ground

**Example**:
> Claim: "Subsidies for new energy vehicles are being actively disbursed at the provincial level."
> Evidence Level: E4 — Supported by central and ministry documents (E3) PLUS provincial fund disbursement notices and project approval records (C-level implementation evidence).

---

### E5: Comprehensive Evidence with Outcome Data

**Definition**: The claim is supported by E4-level evidence PLUS official outcome data that confirms the policy effect has occurred as claimed.

**When to use**:
- E4 evidence exists AND NBS, PBOC, or other official statistics confirm the claimed effect
- E4 evidence exists AND audit or evaluation reports confirm the outcome
- E4 evidence exists AND Supreme People's Court or other judicial data confirms the enforcement trend
...
- Even E5 claims must note limitations (data may have measurement issues, attribution may not be fully causal)

**Example**:
> Claim: "China's NEV production has increased substantially as a result of policy support."
> Evidence Level: E5 — Supported by policy documents (E3), implementation evidence (E4), AND NBS data showing NEV production growth figures.
...
|-----------|----------------------|
| What a document says (original wording) | E2 (single document) |
| What a policy means (interpretation) | E3 (multi-source) or E2 with explicit caveats |
| Implementation status | E4 (implementation verification) |
| Policy effect on the ground | E4 (implementation verification) |
| Market or economic outcome | E5 (outcome data) |
| Future prediction | E1 maximum for specific predictions; E3 for directional trends |

### Rule 3: Evidence Level Determines Confidence Language

| Evidence Level | Allowed Confidence Language |
|---------------|---------------------------|
| E0 | "No evidence; purely speculative" |
| E1 | "Possible but unverified"; "based on indirect signals" |
| E2 | "Supported by one source"; "directionally indicated" |
| E3 | "Supported by multiple sources"; "convergent evidence suggests" |
| E4 | "Supported by multiple sources and implementation evidence"; "actively being implemented" |
| E5 | "Confirmed by policy documents, implementation, and outcome data" |
...
Multiple E1 sources do not create an E3 claim. Multiple E-level sources discussing the same point do not create E2 evidence. Source quality matters more than quantity for evidence level determination.

### Rule 6: Explicit About Missing Links

If the evidence chain is incomplete (e.g., you have E3 evidence but are missing implementation verification), state explicitly what is missing and what evidence level the current chain supports.
