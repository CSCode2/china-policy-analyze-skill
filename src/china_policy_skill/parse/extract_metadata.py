...
class MetadataExtractor:
    TITLE_PATTERNS = [
        ...
        re.compile(
            r'<meta[^>]+(?:property|name)=["\']article:title["\'][^>]+content=["\'](.*?)["\']',
            re.IGNORECASE,
        ),
    ...
    DATE_PATTERNS_HTML = [
        ...
        re.compile(
            r'<meta[^>]+(?:property|name)=["\']date["\'][^>]+content=["\'](.*?)["\']', re.IGNORECASE
        ),
    ...
    AUTHOR_PATTERNS = [
        ...
        re.compile(
            r'<meta[^>]+(?:property|name)=["\']article:author["\'][^>]+content=["\'](.*?)["\']',
            re.IGNORECASE,
        ),
    ]
    ...
    TITLE_SUFFIX_RE = re.compile(
        r"[_—–\-]\s*(中国政府网|最新政策|其他|政策解读|宏观经济|海关|医药管理|新华社|新华网|人民网|求是网|国务院部门网站|地方政策).*$"
    )
    ...
    def _extract_issuing_body(self, html_or_text: str) -> str:
        ...
        issuing_patterns = [
            ...
            re.compile(
                r"(国家发展改革委|工业和信息化部|科学技术部|财政部|商务部|人力资源和社会保障部|自然资源部|生态环境部|住房和城乡建设部|交通运输部|农业农村部|文化和旅游部|国家卫生健康委员会|应急管理部|司法部|民政部|教育部|审计署|国家医疗保障局|国家能源局|海关总署|国家税务总局|国家市场监督管理总局|中国人民银行|中国证监会|国家金融监督管理总局|国家外汇管理局|公安部|最高人民检察院|最高人民法院)"
            ),
        ]
